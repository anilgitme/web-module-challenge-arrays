...
is31Flavors(originalFlavors); // true
// is31Flavors(newArr); false
...
function addFlavor(addItem, array) {

    let newArr = array.unshift(addItem);
    return array;
...
function filterByWord(checkArray, filterWord) {
    let filteredArr = [];

    for (let i = 0; i < checkArray.length; i++) {

        if (checkArray[i].includes(filterWord)) {
            filteredArr.push(checkArray[i]);
